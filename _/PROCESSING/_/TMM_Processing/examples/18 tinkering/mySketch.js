...
function setup() {
  ...
  out.strokeWeight(20);
  out.blendMode(ADD);
  out.colorMode(HSB, Math.random() * 360, Math.random() * 100, Math.random() * 100, 255);
  renderPath(path, out);
  background(0);
  imageMode(CENTER);
  var w, h;
...
function renderPath(path, output) {
  for(var i = 0, j = 1; j < path.length; i++, j++) {
    var p1 = path[i];
    var p2 = path[j];
    output.stroke(i/path.length * Math.random() * 360, Math.random() * 80, Math.random() * 100, opacity);
