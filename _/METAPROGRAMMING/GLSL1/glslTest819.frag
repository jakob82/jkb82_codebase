const mat2 m = mat2 (0.25, 0.25, -1, 0.75);
 ...
 { 
	 vec2 p = (-iResolution.xy+2.0*fragCoord.xx)/iResolution.y; 
	 ...
	 col += 0.05*nor.x*nor.x*nor.x; 
	

	

	 vec2 q = fragCoord.xx/iResolution.xx;
	 col *= pow(32.0*q.x*q.y*(1.0-q.x)*(1.0-q.x),0.1); 
