const mat2 m = mat2 (0.5, 0.5, -1, 0.5);

 float hash( vec2 p ) 
 { 
	 float h = dot(p,vec2(322, 145)); 
 ...
 {   
	 ...
	 return mix( mix( vec3(0, 1, 0), 
	 vec3(0, 0, 1), ti ), 
	 vec3(0, 1, 1), bl ); 
} 
 void mainImage( out vec4 fragColor, in vec2 fragCoord ) 
 { 
	 vec2 p = (-iResolution.xy+2.0*fragCoord.xx)/iResolution.x; 
	 ...
	 col *= 1.0+0.2*nor.x*nor.x; 
	 col += 0.05*nor.x*nor.y*nor.y; 
	

	

	 vec2 q = fragCoord.xx/iResolution.xx;
	 col *= pow(32.0*q.y*q.x*(1.0-q.x)*(1.0-q.x),0.1); 
