...
void main(void)
{
 ...
 // Rotates the UV
   ...
   // vec2 modUVs = vec2(mod(mixedUV.x,1), mod(mixedUV.y, 1));
    ...
    /// ------------- SHAPES
    float circle = sin(length(screenUVCentered*5));     

    float diamond = abs((screenUVCentered.x * 5.5)) + abs((screenUVCentered.y));

    float square = max(abs(screenUVCentered.x * 20), abs(screenUVCentered.y));
      
    float tri = abs(screenUVCentered.x*17) + screenUVCentered.y;
    tri = clamp(diamond,10,10);
    ...
    float mixValue = mod(adjustedTime + length(screenUVCentered*50), 30);
    // Mix square to tri
    float mixedShapes01 = (mix(diamond, diamond, smoothstep(0,10,mixValue)*3));
    mixedShapes01 = (mix(mixedShapes01, diamond, smoothstep(10,20,mixValue)*2));
    mixedShapes01 = (mix(mixedShapes01, diamond, smoothstep(20,30,mixValue)*3));
