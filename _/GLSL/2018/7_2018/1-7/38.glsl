...
void main(void)
{
    float pi = abs(sin(spectrum.x*10));
   ...
   // radianValue += time;
    ...
    float phi_fin = index * pi*3.14 / (segments/30);
    ...
    float offset = fract(modifiedTime) * 9.33 * length(centeredFragCoordNorm * 3366) * 3.314;
   // float w = cos(floor(seg) * freq - modifiedTime + length(centeredFragCoordNorm*offset)) * 4 + 0.1;
    float c = sin((w*spectrum.x*10) / 33 - abs(sin(seg) - 032.2)) * ivr;
  ...
  //  fragColor = vec4(radianValue); 
    ...
    fragColor = vec4(c*abs(sin(spectrum.x*10)));
}
