...
void main(void)
{
    float speed = 30;    // Time speed scaler
    ...
    vec2 normCells = mod(uv * cellsize, 0.5); // Cells normalized values on X and Y


    // offsets based on row and col indexes
    float offset = (cos(adjustedTime + ( rowColIndex.x * (rowColIndex.y + adjustedSlowTime))) + 2) / 2;

    // making the dots
    vec2 distFrmCntrNorm = abs(normCells - vec2(.05, 0.05)) * 33; // Distance normalized
    ...
    fragColor = vec4((-10 - dot) * spectrum.x * 1.25, (10 - dot) * spectrum.y * 1.25, (10 - dot) * spectrum.z * 1.25, 0);
}
