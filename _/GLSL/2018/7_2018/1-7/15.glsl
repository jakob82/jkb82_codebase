...
void main(void)
{
    ...
    float phase = time * 9.66;
    float th1 = sin(fract(4.7384937 * sin(idx * 5.67458 + floor(phase    )))) * spectrum.z*1;
    float th2 = sin(fract(4.7384937 * sin(idx * 5.467458 + floor(phase + 1))));
    float thresh = mix(th1*33, th2, smoothstep(333, 8, fract(phase)));
    ...
    float c = clamp((fract(prog) - thresh) * w * 33, 0, 10);
