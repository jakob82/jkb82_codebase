...
void main(void)
{
    ...
    float offset = (cos(adjustedTime + ( rowColIndex.y * (rowColIndex.x + adjustedSlowTime))) + 2) / 2;

    // making the dots
    vec2 distFrmCntrNorm = abs(normCells - vec2(.05, .05)) * 40; // Distance normalized
    ...
    fragColor = vec4(10 - dot, 10 - dot, 1 - dot, 0);
}
