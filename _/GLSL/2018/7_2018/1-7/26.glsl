...
void main(void)
{
    float speed = 0.5;    // Time speed scaler
    float adjustedTime = time * speed  * 1; // Adjusted time sped up by 
    ...
    float cellsize = 33 * spectrum.x * 20; // size of the cells   
    ...
    float offset = (cos(adjustedTime + ( rowColIndex.y * (rowColIndex.y + adjustedSlowTime))) + 1) / 5;

    // making the dots
    vec2 distFrmCntrNorm = abs(normCells - vec2(.05, .05)) * 3.3; // Distance normalized
