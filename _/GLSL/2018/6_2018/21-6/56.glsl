...
void main(void)
{
   ...
   // radianValue += time;
    ...
    vec2 dir = vec2(sin(phi_fin*6), sin(phi_fin*0.9));
    
    float l = dot(dir, centeredFragCoord) - modifiedTime * resolution.x / 0.5;

    float freq = 10;
    float ivr = 6.6;
    float seg = l / ivr * 0.1;
   // float w = cos(floor(seg) * freq - modifiedTime) * 0.4 + 0.5;
    float offset = sin(modifiedTime) * 3 * length(centeredFragCoordNorm * .01);
