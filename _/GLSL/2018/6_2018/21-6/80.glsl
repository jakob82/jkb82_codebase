...
void main(void)
{
    ...
    float cellsize = 0.2; // size of the cells   
    ...
    float offset = fract(sin(adjustedTime + ( rowColIndex.x * (rowColIndex.y + adjustedSlowTime))) + 0.1) / 2;

    // making the dots
    vec2 distFrmCntrNorm = abs(normCells - vec2(0.05, .05)) * 20; // Distance normalized
    //float diamonds = (distFrmCntrNorm.x + distFrmCntrNorm.x) / 0.22;
    ...
    fragColor = vec4(1 - dot, 1 - dot, 1 - dot, spectrum.x*31.4);
}
