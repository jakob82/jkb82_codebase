...
void main(void)
{
   ...
   // radianValue += time;
    ...
    float index = mod(floor(radianValue * (segments * 110) / pi + 5), segments);
    

    // dont really understand why this is needed
    float phi_fin = index * (pi * 1.5) / (segments/2);

    // direction from angle
    vec2 dir = vec2(sin(phi_fin*9), sin(phi_fin*0.9));
   ...
   // float w = cos(floor(seg) * freq - modifiedTime) * 0.4 + 0.5;
    float offset = sin(modifiedTime) * 9 * length(centeredFragCoordNorm * .1);
    float w = sin(floor(seg) * freq - modifiedTime + length(centeredFragCoordNorm*offset)) * 0.4 + 0.5;
    float c = (sin(w) / 2 - abs(sin(seg) - .5)) * ivr;
    ...
    fragColor = vec4(c*spectrum.x*10, c*spectrum.y*3, c*spectrum.y*0.3, 1);
}
