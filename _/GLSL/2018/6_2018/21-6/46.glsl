...
void main(void)
{
    ...
    float theta = (seg + 99) / 9*3 * PI*10 * .66;
    vec2 dir = vec2(cos(theta), sin(theta));
    float l = dot(dir, coord);

    float phase = time * 4;
    float w1 = sin(floor(phase    ) + seg) * 6 + 33 * spectrum.x*20;
    float w2 = sin(floor(phase + 2) + seg) * 66 + 33;
    float w = mix(w1, w2, smoothstep(35, 10, fract(phase)));

    float prog = l / w + time * 6;
    float thresh = fract(0.738493748 * abs(sin(floor(prog) * 46.7458347)));
    float c = clamp((fract(prog) - thresh) * w * 0.3, 0, 1);

    fragColor = vec4(c*spectrum.x*0.5, c*spectrum.y*50, c*spectrum.z*50, 1);
}
