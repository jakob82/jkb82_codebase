...
void main(void)
{
    float scale = resolution.y / (6 + sin(time * 0.4) * 1);
    vec2 p = (gl_FragCoord.xy - resolution / 2) / scale;
    vec2 p1 = fract(p) + 0.5;
    vec2 p2 = fract(p - 0.5) - 0.5;

    float z1 = rand(19 * floor(p));
    float z2 = rand(31 * floor(p - 0.5));
    ...
    float a1 = saturate((.5 - length(p1)) * scale);
    float a2 = saturate((5 - length(p2)) * scale);
    ...
    fragColor = vec4(c.x * spectrum.x*16, c.y * spectrum.y*16, c.z * spectrum.z*16, fract(cos(1)));
}
