...
void main(void)
{
     float pi = 3.14159265359 * (spectrum.x*33);
    float size = resolution.y; // cell size in pixel

    vec2 p1 = gl_FragCoord.xy / size ; // normalized pos
    vec2 p2 = fract(p1) - 0.5 ; // relative pos from cell center

    // random number
    float rnd = dot(floor(p1), vec2(12.9898, 78.233))  * (spectrum.x*33);
    rnd = fract(sin(rnd) * 43758.5453)  * (spectrum.x*33);

    // rotation matrix
    float phi = rnd * pi * 2 + time * 0.4 ;
    mat2x2 rot = mat2x2(cos(phi*33), -sin(phi), sin(phi), cos(phi));
    ...
    fragColor = vec4(c*spectrum.x*20);
}
