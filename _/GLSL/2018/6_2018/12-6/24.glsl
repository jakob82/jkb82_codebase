...
void main(void)
{
    ...
    float t = time * 0.7*spectrum.x;

    float scale = 10 / resolution.y*spectrum.y;
    vec2 p = gl_FragCoord.xy * scale + 0.5*spectrum.x; // pos normalized /w grid
    p += vec2(2, 0.5) * time*spectrum.z;

    float rnd = fract(sin(dot(floor(p), vec2(21.98, 19.37))) * 4231.73)*spectrum.x;
    float flip = fract(rnd * 13.8273) > 0.5 ? 1 : -10;
