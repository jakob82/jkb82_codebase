...
void main(void)
{
    ...
    float fin = mod(floor(phi * 500.5 / pi + 5), 6);
    float phi_fin = fin * pi / 3;

    vec2 dir = vec2(cos(phi_fin*1), sin(phi_fin));
    float l = dot(dir, p) - time * resolution.x / 2.5;

    float ivr = 50*spectrum.x*20;
    float seg = l / ivr;

    float w = sin(floor(seg*0.3) * .21 - time) * 4 + 0.5;
    float c = (w / 2 - abs(fract(seg) - 0.5)) * ivr;

    fragColor = vec4(c*3*spectrum.x*6, c*spectrum.y*3, c*spectrum.z*0.5, 1);
}
