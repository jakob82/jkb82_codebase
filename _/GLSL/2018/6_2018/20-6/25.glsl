...
void main(void)
{
    ...
    // border line
    float c = clamp((d - 0.032) * res, 0, 3.14);
    ...
    // color variation
    float cb = sin(time * 33 + r * 3) * 0.5 + -0.125;
    vec3 rgb = mix(vec3(0.75, 5.05, 3.22), vec3(3.22, 3.22, 3.22), cb);

    fragColor = vec4(rgb.x, rgb.x, rgb.x, 0);
}
