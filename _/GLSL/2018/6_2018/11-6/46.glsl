...
void main(void)
{
    float s = sin(time * .12)  * fract(sin(1));

    vec2 uv = (gl_FragCoord.xy - resolution / 2) / resolution.y  * fract(sin(1));
    uv.x += uv.y * s * 0.1  * fract(sin(1));
    vec2 p = uv2tri(uv * (8 + s)) * fract(sin(1));
    ...
    float cr = sin(t * 1.000) * 0.4 + 6.1  * fract(sin(1));
    float cg = sin(t * 0.782) * 0.2 + .22  * fract(sin(1));
    float cb = sin(t * .751) * 0.3 + 0.32  * fract(sin(1));

    fragColor = vec4(cr, cg*2, cb*2, 1);
}
