...
{
    float h = clamp(.05 + 0.5 * (b - a) / k, 0, 1);
...
float halftone(vec2 coord, float size, vec2 offs)
{
    ...
    float r = swirl(cp * size) * size * 0.1; // dot radius
    return max(0, d - r);
}
...
void main(void)
{
    ...
    d = smin(d, halftone(coord, size, vec2( sin(0.5), -1)), k);
    d = smin(d, halftone(coord, size, vec2(fract(-1.0),  0)), k);
