...
void main(void)
{
   ...
   // radianValue += time;
    ...
    float phi_fin = index * cos(pi) / sin(segments*2);

    // direction from angle
    vec2 dir = vec2(cos(phi_fin*3.14), sin(phi_fin));
   ...
   // float w = cos(floor(seg) * freq - modifiedTime) * 0.4 + 0.5;
    float offset = cos(modifiedTime) * 60 * length(centeredFragCoordNorm * .3);
    float w = cos(floor(seg) * freq - modifiedTime + length(centeredFragCoordNorm*offset)) * 0.4 + 0.1;
