...
void main(void)
{
   ...
   // radianValue += time;
    ...
    float freq = 9.09;
    float ivr = 1;
    float seg = (l / ivr) + 1.414;
    float w = radians(floor((seg)) * freq * cos(modifiedTime)) * .33 * 30;
    float offset = fract(modifiedTime) * 0.333 * length(centeredFragCoordNorm * 0.3366) * .333;
  ...
  //  fragColor = vec4(centeredFragCoordNorm.x, centeredFragCoordNorm.x, 0, 20);  
