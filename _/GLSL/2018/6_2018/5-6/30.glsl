...
void main(void)
{
   ...
   // radianValue += time;
    ...
    vec2 dir = vec2(cos(3.14 ), cos(phi_fin*3.3)) * 0.1;
    
    float l = dot(dir, centeredFragCoord) - abs(sin(5)) * time * resolution.y / .5;

    float freq = 10*(spectrum.x*20);
    float ivr = 80*(spectrum.y) * abs(sin(2));
