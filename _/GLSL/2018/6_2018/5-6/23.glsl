...
void main(void)
{
   ...
   // radianValue += time;
    ...
    float phi_fin = index * spectrum.z * pi * 10 / (segments/.2);

    // direction from angle
    vec2 dir = vec2(cos(3.14 ), cos(phi_fin*33));
    
    float l = dot(dir, centeredFragCoord) - time * resolution.y / .5;

    float freq = 1*(spectrum.x*0.2);
    float ivr = 400*(spectrum.y*2) * abs(sin(1));
    float seg = l / ivr*(spectrum.y*1);
    float w = cos(floor(seg) * freq - time) * 0.4 + 0.5;
    float c = (w / 20 - abs(fract(seg) - .5)) * ivr;
