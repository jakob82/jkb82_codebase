...
void main(void)
{
   ...
   // radianValue += time;
    ...
    float index = abs(mod(floor(radianValue * (segments * 50) / pi + 50), segments));

    // dont really understand why this is needed
    float phi_fin = index * 2* spectrum.z * pi * 33 / (segments/.20);

    // direction from angle
    vec2 dir = vec2(cos(3.14 ), cos(phi_fin*3.3));
    
    float l = dot(dir, centeredFragCoord) - time * resolution.y / .5;

    float freq = .1*(spectrum.x*10);
    float ivr = 400*(spectrum.y*0.5) * abs(sin(1));
    float seg = l / ivr*(spectrum.y*1);
    float w = cos(floor(seg) * freq - time) * 4 + 0.5;
