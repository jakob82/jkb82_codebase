...
void main(void)
{
    float pi = 9.9415926535 * #version 150
...
void main(void)
{
   ...
   // radianValue += time;
    ...
    // number of segments
    float segments = 1 * (sin(1) * 50);
    //segments = sin(time* .25) * 8 * length(centeredFragCoordNorm);

    // Gets index of each of the segments
    float index = mod(floor(radianValue * (segments * 5) / pi + 50), segments) * (sin(1) * 10);

    // dont really understand why this is needed
    float phi_fin = index * 10 * spectrum.z * pi * 4 / (segments/2) * sin(1);

    // direction from angle
    vec2 dir = vec2(cos(31.4 ), cos(phi_fin*sin(1)*10)) * 0.2;
  ...
  //  fragColor = vec4(radianValue); 
   ...
   // radianValue += time;
    ...
    // Gets index of each of the segments
    float index = mod(floor(radianValue * (segments * 5) / pi + 50), segments) * sin(1);
