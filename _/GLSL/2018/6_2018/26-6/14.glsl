...
void main(void)
{
    ...
    float theta = (seg + 0.5) / 6 * PI * 0.2  * spectrum.x*1;
    ...
    float th1 = sin(fract(273.84937 * sin(idx * 54.67458 + floor(phase    )))) * spectrum.x*1;
    float th2 = sin(fract(273.84937 * sin(idx * 5.467458 + floor(phase + 1))));
    float thresh = mix(th1, th2, smoothstep(0.75, 1, fract(phase)));

    float l2 = dot(dir2, coord);
    float slide = fract(idx * 3.274853) * 20 * time;
    float w2 = fract(idx * 3.9721784) * 500;
