...
void main(void)
{
   ...
   // radianValue += time;
    ...
    vec2 dir = vec2(radians(phi_fin), cos(phi_fin)) * 0.1;
    ...
    float w = radians(floor((seg)) * freq - cos(modifiedTime)) * 8.8 + 15.0;
    float offset = fract(modifiedTime) * 33.3 * length(centeredFragCoordNorm * 33.66) * 33.3;
   // float w = cos(floor(seg) * freq - modifiedTime + length(centeredFragCoordNorm*offset)) * 8 + 0.05;
    float c = sin((w) / 322 - abs(radians(seg) - 0.322)) * ivr;
