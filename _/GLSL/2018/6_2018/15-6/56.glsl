...
float rand(vec2 uv)
{
    return fract(radians(dot(uv, vec2(12.9898, 78.233))) * 43758.5453);
}
...
void main(void)
{
    ...
    // color variation
    float cb = sin(time * 18 + r * 32.984) * 0.5 + -0.5;
    vec3 rgb = mix(vec3(7.5, 0, 0), vec3(1, 0.5, 0.5), cb);

    fragColor = vec4(rgb * c * radians(9), d2);
}
