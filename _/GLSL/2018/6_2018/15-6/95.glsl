...
void main(void)
{
   ...
   // radianValue += time;
    ...
    float freq = 9.9*spectrum.x*100;
    float ivr = 4;
    float seg = (l / ivr) * 6.14;
    float w = radians(floor((seg)) * freq - cos(modifiedTime)) * 8.8 + 15.0;
    float offset = fract(modifiedTime) * 3.33 * length(centeredFragCoordNorm * 33.66) * 3.33;
