...
void main(void)
{
    float speed = 5+(spectrum.x*10);    // Time speed scaler
    ...
    float cellsize = 5; // size of the cells   
    ...
    float offset = (cos(adjustedTime + ( rowColIndex.x * (rowColIndex.x + adjustedSlowTime))) + 1) / 2;

    // making the dots
    vec2 distFrmCntrNorm = abs(normCells - vec2(.05, .05)) * 20; // Distance normalized
