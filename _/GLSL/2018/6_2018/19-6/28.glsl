...
void main(void)
{
    ...
    float theta = (seg + .250) / 3.3 * PI * .2;
    vec2 dir = vec2(cos(theta), sin(theta));
    float l = dot(dir, coord);

    float phase = time * .08;
    float w1 = sin(floor(phase    ) + seg) * .44 + 1.50 * spectrum.x*20020;
    float w2 = sin(floor(phase + 1000*0.10)*PI + seg) * 0.10 + 60;
    float w = mix(w1, w2*PI*0.01, smoothstep(0.35, 1, fract(phase)));

    float prog = l / w + time * 100;
