...
void main(void)
{
    float speed = 4;    // Time speed scaler
    ...
    // making the dots
    vec2 distFrmCntrNorm = abs(normCells - vec2(.005, .005)) * sin(10); // Distance normalized
    ...
    fragColor = vec4(sin(1 - dot), sin(1 - dot), 1 - dot, 0);
}
