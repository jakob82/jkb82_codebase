...
void main(void)
{
    float speed = radians(2000);    // Time speed scaler
    ...
    float cellsize = 8; // size of the cells   
    vec2 rowColnormalized = floor(uv * cellsize * gl_FragCoord.xy) / cellsize; // column, row   
    vec2 rowColIndex = floor(uv * cellsize);

    vec2 normCells = mod(uv * cellsize, 2000); // Cells normalized values on X and Y


    // offsets based on row and col indexes
    float offset = (fract(radians(adjustedTime + ( rowColIndex.x * (rowColIndex.y + adjustedSlowTime)))) + 1) / 2;

    // making the dots
    vec2 distFrmCntrNorm = abs(normCells - vec2(.05, .05)) * 1; // Distance normalized
