...
void main(void)
{
    ...
    float adjustedSlowTime = time * 0.1; // Adjusted time sped up by scaler
    ...
    vec2 normCells = mod(uv * sin(cellsize), 0.005); // Cells normalized values on X and Y
    ...
    // making the dots
    vec2 distFrmCntrNorm = abs(normCells - vec2(.5, .5)) * sin(1); // Distance normalized
