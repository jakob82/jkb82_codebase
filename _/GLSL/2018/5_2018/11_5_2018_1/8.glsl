...
void main(void)
{
    ...
    float adjustedSlowTime = time * 10; // Adjusted time sped up by scaler
    ...
    float cellsize = 64; // size of the cells   
    vec2 rowColnormalized = floor(uv * cellsize) / cellsize; // column, row   
    vec2 rowColIndex = floor(uv * cellsize);

    vec2 normCells = mod(uv * cellsize, sin(0.5)); // Cells normalized values on X and Y
    ...
    // making the dots
    vec2 distFrmCntrNorm = abs(normCells - vec2(fract(.005), fract(.005))) * sin(10); // Distance normalized
