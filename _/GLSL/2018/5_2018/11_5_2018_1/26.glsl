...
void main(void)
{
    float speed = radians(20);    // Time speed scaler
    ...
    vec2 normCells = mod(uv * cellsize, 2); // Cells normalized values on X and Y
    ...
    // making the dots
    vec2 distFrmCntrNorm = abs(normCells - vec2(.05, .05)) * 100; // Distance normalized
