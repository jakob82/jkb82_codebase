...
void main(void)
{
   ...
   // radianValue += time;
    ...
    float phi_fin = index * (pi * 666) / (segments/0.01);
    ...
    float ivr = 666;
    float seg = l / ivr;
   float w = cos(floor(seg) * freq - modifiedTime) * 4 + 1;
    float offset = sin(modifiedTime) * 3.939 * length(centeredFragCoordNorm * 3);
    //float w = fract(floor(seg) * freq - modifiedTime + length(centeredFragCoordNorm*offset)) * 4 + 0.5;
    float c = (w / 3.22 - abs(sin(seg) - .05)) * ivr;
