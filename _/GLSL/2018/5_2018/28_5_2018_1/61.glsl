...
void main(void)
{
    ...
    float radianValue = atan(centeredFragCoord.y, centeredFragCoord.x)*spectrum.x;
    //radianValue += centeredFragCoordNorm.x * sin(time) * 2 * length(centeredFragCoordNorm)*2;

   // radianValue += time;

    // number of segments
    float segments = 9*spectrum.z;
    //segments = sin(time* .25) * 8 * length(centeredFragCoordNorm);

    // Gets index of each of the segments
    float index = mod(floor(radianValue * (segments * 5) / pi + 5), segments)*spectrum.x;
    ...
    float freq = 32*spectrum.x;
    float ivr = 3*spectrum.x;
    float seg = (l / ivr) * .314*spectrum.z;
