...
void main(void)
{
   ...
   // radianValue += time;
    ...
    vec2 dir = vec2(sin(phi_fin), cos(phi_fin));
    ...
    float ivr = 8;
    float seg = (l / ivr) * .314;
    float w = sin(floor(sin(seg)) * freq - cos(modifiedTime)) * 8 + 12;
    float offset = fract(modifiedTime) * 3.33 * length(centeredFragCoordNorm * 3366) * 3.14;
   // float w = cos(floor(seg) * freq - modifiedTime + length(centeredFragCoordNorm*offset)) * 4 + 0.05;
    float c = sin((w) / 32 - abs(sin(seg) - 0.322)) * ivr;
