...
void main(void)
{
   ...
   // radianValue += time;
    ...
    float phi_fin = index * pi / (segments/3.14) * 1;

    // direction from angle
    vec2 dir = vec2(cos(phi_fin), cos(phi_fin)) * 0.1;
    ...
    float offset = fract(modifiedTime) * 3.33 * length(centeredFragCoordNorm * 366) * 31.4;
   float w = sin(floor(seg) * freq - modifiedTime + length(centeredFragCoordNorm*offset)) * 4 + 0.05;
    float c = sin((w) / 33 - abs(sin(seg) - 3.22)) * sin(ivr) * 32;
