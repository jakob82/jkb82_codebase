...
void main(void)
{
   ...
   // radianValue += time;
    ...
    float phi_fin = index * pi / (segments/20);

    // direction from angle
    vec2 dir = vec2(fract(phi_fin), sin(phi_fin));
   ...
   // float w = cos(floor(seg) * freq - modifiedTime) * 0.4 + 0.5;
    float offset = fract(modifiedTime) * 31.4 * length(centeredFragCoordNorm * .003);
    float w = cos(floor(seg) * freq - modifiedTime + length(centeredFragCoordNorm*offset)) * 40 + 0.05;
