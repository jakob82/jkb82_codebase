...
void main(void)
{
   ...
   // radianValue += time;
    ...
    float index = mod(floor(radianValue * (segments * 500) / pi + 0.5), segments);
    ...
    vec2 dir = vec2((phi_fin), cos(phi_fin));
   ...
   // float w = cos(floor(seg) * freq - modifiedTime) * 0.4 + 0.5;
    float offset = sin(modifiedTime) * 31.4 * length(centeredFragCoordNorm * .003);
    float w = sin(floor(seg) * freq - modifiedTime + length(centeredFragCoordNorm*offset)) * 40 + 0.05;
