...
void main(void)
{
   ...
   radianValue += time;
    ...
    float index = mod(floor(radianValue * (segments * 777777) / pi + 1555.5), segments);
    

    // dont really understand why this is needed
    float phi_fin = index * (pi * 55555) / (segments/33333);
    ...
    float c = (cos(sin(w)) / sin(20) - abs(sin(seg) - .1)) * (sin(ivr)*999999);
