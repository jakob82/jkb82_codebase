...
void main(void)
{
   ...
   // radianValue += time;
    ...
    float phi_fin = index * -(pi * -.32) / (segments/55);

    // direction from angle
    vec2 dir = vec2(fract(-phi_fin), fract(-phi_fin));
    
    float l = dot(dir, centeredFragCoord) - modifiedTime * resolution.y / 0.5;

    float freq = .1;
    float ivr = sin(1)*cos(1.2);
    float seg = l / ivr;
   // float w = cos(floor(seg) * freq - modifiedTime) * 0.4 + 0.5;
    float offset = fract(modifiedTime) * 0.99 * length(centeredFragCoordNorm * .3);
    float w = fract(floor(seg) * freq - modifiedTime + length(centeredFragCoordNorm*offset)) * 0.4 + 0.5;
    float c = (w / 2 - abs(sin(seg) - .005)) * ivr;
