...
void main(void)
{
   ...
   // radianValue += time;
    ...
    vec2 dir = vec2(cos(phi_fin * 0.314), sin(phi_fin));

    
    float l = sin(dot(dir, centeredFragCoord) - fract(time) * fract(resolution.y) / 50);

    float freq = .01;
    float ivr = 200;
    float seg = l / ivr;
    float w = abs(sin(sin(floor(seg)) * sin(freq - time))) * fract(sin(0.4) + sin(0.5));
    float c = abs((sin(w / 2) - abs(fract(seg) - 0.5)) * sin(ivr))*1;
