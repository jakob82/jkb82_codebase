...
void main(void)
{
 ...
 // Rotates the UV
   ...
   // vec2 modUVs = vec2(mod(mixedUV.x,1), mod(mixedUV.y, 1));
    ...
    float diamond = abs(screenUVCentered.x * 2) + abs(screenUVCentered.y);

    float square = max(abs(screenUVCentered.x * 2), abs(screenUVCentered.y));
    ...
    // Mix square to tri
    float mixedShapes01 = mix(diamond, diamond, smoothstep(0,1,mixValue)*0.3);
