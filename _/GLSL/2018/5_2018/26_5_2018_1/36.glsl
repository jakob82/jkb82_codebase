...
void main(void)
{
   ...
   // radianValue += time;
    ...
    vec2 dir = vec2(cos(phi_fin * 314000), sin(phi_fin));

    
    float l = sin(dot(dir, centeredFragCoord) - fract(time) * fract(resolution.y) / 500);

    float freq = 10;
    float ivr = 200;
    float seg = l / ivr;
    float w = abs(sin(sin(floor(seg)) * sin(freq - time))) * fract(sin(0.4) + sin(0.5));
    float c = abs((sin(w / 4) - abs(fract(seg) - 1)) * sin(ivr))*1;
