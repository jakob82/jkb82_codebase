...
void main(void)
{
    float speed = abs(sin(80));    // Time speed scaler
    ...
    vec2 coord = sin(gl_FragCoord.xy); // screen coord
    ...
    vec2 rowColnormalized = (fract(sin(floor(uv * cellsize) / cellsize))); // column, row   
    vec2 rowColIndex = floor(uv * cellsize);

    vec2 normCells = (abs(mod(sin(uv) * cellsize, 0.5))); // Cells normalized values on X and Y


    // offsets based on row and col indexes
    float offset = ((adjustedTime + ( rowColIndex.x * (rowColIndex.y + adjustedSlowTime))) + 1) / 2;
