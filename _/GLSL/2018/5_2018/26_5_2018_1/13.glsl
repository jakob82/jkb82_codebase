...
void main(void)
{
   ...
   // radianValue += time;
    ...
    float index = mod(floor(radianValue * (segments * 50) / pi + 0.05), segments);
    

    // dont really understand why this is needed
    float phi_fin = index * (pi * 320) / (segments/550);
   ...
   // float w = cos(floor(seg) * freq - modifiedTime) * 0.4 + 0.5;
    float offset = sin(modifiedTime) * 99 * length(centeredFragCoordNorm * .03);
