...
void main(void)
{
 ...
 // Rotates the UV
   ...
   // vec2 modUVs = vec2(mod(mixedUV.x,1), mod(mixedUV.y, 1));
    ...
    // Mix square to tri
    float mixedShapes01 = mix(circle, diamond, smoothstep(0,1,mixValue)*0.3);
    mixedShapes01 = mix(mixedShapes01, square, smoothstep(1,2,mixValue)*0.2);
    mixedShapes01 = mix(mixedShapes01, circle, smoothstep(2,3,mixValue)*0.3);

    mixedShapes01 *= 30 * length(screenUVCentered);
    mixedShapes01 = step(sin(sin(mixedShapes01+time*.025)), .05);
