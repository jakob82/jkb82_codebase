...
void main(void)
{
    ...
    // making the dots
    vec2 distFrmCntrNorm = abs(normCells - vec2(.05, .05)) * 2; // Distance normalized
    ...
    dot = abs(sin(fract(pow(dot, 4 * ( offset) )))); // 
    dot = sin(fract(sin(floor(dot * 1.2))));


    //fragColor = vec4(distFrmCntr.x,distFrmCntr.y, 0, 0); // dist from cell center
    fragColor = vec4(1 - dot, 1 - dot, 10 - dot, 0);
}
