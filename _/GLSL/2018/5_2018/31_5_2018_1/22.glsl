...
void main(void)
{
    float pi = 31.415926535 * 0.1;
    float modifiedTime = time * 20 * 10;
   ...
   // radianValue += time;
    ...
    // number of segments
    float segments = 300000;
    //segments = sin(time* 2.5) * 80 * length(centeredFragCoordNorm) * spectrum.x;
    ...
    vec2 dir = vec2(cos(3.14 ), cos(time*0.5)) * 10 * spectrum.x*1000;
    
    float l = dot(dir, centeredFragCoord) - time * resolution.y / sin(5);

    float freq = .01*(spectrum.x*100);
    float ivr = 400*(spectrum.y*0.1);
  ...
  //  fragColor = vec4(radianValue); 
    ...
    fragColor = vec4(0, c*time, c*time, c*time);
}
