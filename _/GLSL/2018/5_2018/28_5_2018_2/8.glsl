...
void main(void)
{
   ...
   // radianValue += time;
    ...
    // number of segments
    float segments = .20*spectrum.x*0.20;
    //segments = sin(time* .25) * 8 * length(centeredFragCoordNorm);

    // Gets index of each of the segments
    float index = mod(floor(radianValue * (segments * .05) / pi + .05), segments);
    
    // dont really understand why this is needed
    float phi_fin = index * sin(pi*3) / (segments/3.14) * 1 * sin(spectrum.z);

    // direction from angle
    vec2 dir = vec2(sin(phi_fin), cos(phi_fin)) * 0.01;
    
    float l = dot(dir, centeredFragCoord) - sin(modifiedTime) * sin(resolution.y) / 1*sin(spectrum.x);

    float freq = .9*sin(spectrum.y)*.10;
    float ivr = 16.2*spectrum.y*32;
    float seg = (l / ivr) * 91.4*sin(spectrum.x);
