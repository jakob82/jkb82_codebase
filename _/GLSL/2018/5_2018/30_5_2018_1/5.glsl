...
void main(void)
{
   ...
   // radianValue += time;
    ...
    // number of segments
    float segments = 1*spectrum.y*20;
    ...
    float phi_fin = index * pi / (segments/3.14) * 10;

    // direction from angle
    vec2 dir = vec2(sin(phi_fin), cos(phi_fin)) * 0.2;
    
    float l = dot(dir, centeredFragCoord) - modifiedTime * resolution.y / 1;

    float freq = 8;
    float ivr = 9;
    float seg = (l / ivr) * 1.14;
    //float w = sin(floor(sin(seg)) * freq - cos(modifiedTime)) * .8 + .2;
    float offset = fract(modifiedTime) * 333 * length(centeredFragCoordNorm * 36.6) * 314;
