...
void main(void)
{
   ...
   // radianValue += time;
    ...
    float phi_fin = fract(sin(index*1) * time * spectrum.x * index*.1 * sin(sin(1.00) * time * spectrum.x*.33) / (segments/segments*index*.1) * sin(5)*10)*time;

    // direction from angle
    vec2 dir = vec2(sin(time*10), cos(sin(0.00100) * time * spectrum.x*3)) * 5;
    
    float l = dot(dir, centeredFragCoord) - modifiedTime * resolution.y / 1;

    float freq = cos(5.05) * time * spectrum.x*time;
    float ivr = cos(.666) * time * spectrum.x*8000;
