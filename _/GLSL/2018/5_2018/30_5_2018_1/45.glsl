...
void main(void)
{
    float pi = sin(5) * time * spectrum.x * time*0.1;
   ...
   // radianValue += time;
    ...
    float phi_fin = fract(sin(index*0.1) * time * spectrum.x * index*0.1 * sin(sin(1.00) * time * spectrum.x*.33) / (segments/segments*index*0.1) * sin(5)*10)*time;
    ...
    float ivr = cos(6.66) * time * spectrum.x*80.000;
