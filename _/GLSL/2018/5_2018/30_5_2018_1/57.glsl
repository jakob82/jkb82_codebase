...
void main(void)
{
   ...
   // radianValue += time;
    ...
    vec2 dir = vec2(sin(time*10), cos(sin(0.00100) * time * spectrum.x*3)) * 0.05;
    
    float l = dot(dir, centeredFragCoord) - modifiedTime * resolution.y / 1*10;

    float freq = cos(5.05) * time * spectrum.x*time*2;
    float ivr = cos(66.6) * time * spectrum.x*2;
