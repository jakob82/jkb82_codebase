...
void main(void)
{
   ...
   // radianValue += time;
    ...
    float phi_fin = (cos(index*1) * time * spectrum.x * index*.1 * sin(sin(1.00) * time * spectrum.x*.33) / (segments/segments*index*.1) * sin(5)*0.10)*time*10*time;
    ...
    float freq = cos(5.05) * time * spectrum.x*time*4;
    float ivr = cos(66.6) * time * spectrum.x*2*time;
   ...
   float w = fract(floor(seg) * sin(freq) * time * spectrum.x*.8*freq - modifiedTime + length(centeredFragCoordNorm*offset)) * sin(100) * time * spectrum.x*10 + sin(33) * time * spectrum.x*time*sin(ivr);
