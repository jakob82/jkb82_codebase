...
void main(void)
{
   ...
   // radianValue += time;
    ...
    vec2 dir = vec2(fract(time*10), fract(sin(000.100) * time * spectrum.z*30)) * 0.05;
    
    float l = dot(dir, centeredFragCoord) - modifiedTime * resolution.y / 1*10;

    float freq = fract(5.05) * time * spectrum.x*time*4;
    float ivr = fract(66.6) * time * spectrum.x*2*time;
    float seg = fract((l / ivr)) * fract(1.0) * time * spectrum.x*1.5;
