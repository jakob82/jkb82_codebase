...
void main(void)
{
   ...
   // radianValue += time;
    ...
    float phi_fin = fract(index * sin(pi*(time*2)) / (segments/114 * (time*.1)) * sin(.05)*2.0);

    // direction from angle
    vec2 dir = vec2(sin(time*2), cos(phi_fin*(time*10))) * 0.5;
    
    float l = dot(dir, centeredFragCoord) - modifiedTime * resolution.y / 1;

    float freq = 2*(time*10);
    float ivr = 40;
    float seg = sin((l / ivr))*(time) * 11.4;
   ...
   float w = sin(floor(seg) * freq - modifiedTime + length(centeredFragCoordNorm*offset)) * 12.0 + .5 * (time*0.1);
    float c = sin((w) / 666 - abs(sin(seg*(time*10)) - 3.22)) * sin(ivr) * 32;
