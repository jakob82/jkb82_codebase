...
void main(void)
{
   ...
   // radianValue += time;
    ...
    vec2 dir = vec2(sin(1), cos(sin(0.100) * time * spectrum.x*3)) * 0.5;
    
    float l = dot(dir, centeredFragCoord) - modifiedTime * resolution.y / 1;

    float freq = cos(505) * time * spectrum.x*4;
    float ivr = cos(6.66) * time * spectrum.x*8;
   ...
   float w = fract(floor(seg) * sin(freq) * time * spectrum.x*.8*freq - modifiedTime + length(centeredFragCoordNorm*offset)) * sin(100) * time * spectrum.x*10 + sin(33) * time * spectrum.x*10;
    float c = sin((w) / 66.6 - abs(sin(seg) - 3.22)) * sin(ivr) * 32;
