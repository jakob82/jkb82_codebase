...
void main(void)
{
    ...
    float modifiedTime = time * sin(100) * time * spectrum.x * spectrum.x;
   ...
   // radianValue += time;
    ...
    float phi_fin = fract(sin(index) * time * spectrum.x * index * sin(sin(100) * time * spectrum.x*3) / (segments/segments*index) * sin(5)*10);

    // direction from angle
    vec2 dir = vec2(sin(1), cos(sin(100) * time * spectrum.x*3)) * 0.5;
    
    float l = dot(dir, centeredFragCoord) - modifiedTime * resolution.y / 1;

    float freq = sin(100) * time * spectrum.x*4;
    float ivr = sin(10) * time * spectrum.x*8;
    float seg = sin((l / ivr)) * sin(100) * time * spectrum.x*.1;
    //float w = sin(floor(sin(seg)) * freq - cos(modifiedTime)) * .8 + .2;
    float offset = sin(modifiedTime) * .333 * length(centeredFragCoordNorm * .366) * .314;
   float w = sin(floor(seg) * sin(freq) * time * spectrum.x*8*freq - modifiedTime + length(centeredFragCoordNorm*offset)) * sin(10) * time * spectrum.x*10 + sin(10) * time * spectrum.x*1;
