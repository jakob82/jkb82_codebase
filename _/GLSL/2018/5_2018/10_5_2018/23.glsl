...
void main(void)
{
    float speed = 0.01;    // Time speed scaler
    ...
    float cellsize = 80; // size of the cells   
    ...
    float offset = (sin(adjustedTime + ( rowColIndex.x * (rowColIndex.y + adjustedSlowTime))) + 1) / 2;

    // making the dots
    vec2 distFrmCntrNorm = abs(normCells - vec2(.5, .5)) * 1.65; // Distance normalized
    ...
    fragColor = vec4(1 - dot, 1 - dot, 0.1 - dot, 0);
}
