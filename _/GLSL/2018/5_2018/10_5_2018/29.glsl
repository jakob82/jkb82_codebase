...
void main(void)
{
    float speed = 0.0001;    // Time speed scaler
    ...
    float cellsize = 100; // size of the cells   
    vec2 rowColnormalized = floor(uv * cellsize) / cellsize; // column, row   
    vec2 rowColIndex = floor(uv * cellsize);

    vec2 normCells = mod(uv * cellsize, 0.05); // Cells normalized values on X and Y
    ...
    // making the dots
    vec2 distFrmCntrNorm = abs(normCells - vec2(0.0005, .0005)) * 1.65; // Distance normalized
